from job_kg.graph import build_knowledge_graph, normalize_jobs
from job_kg.repository import Neo4jGraphRepository


def test_normalize_jobs_and_indexes():
    raw_jobs = [
        {
            "_job_id": "job-1",
            "jobName": "机器学习工程师",
    ...
    assert "机器学习" in graph.indexes["skills"]
    assert len(graph.nodes) > 2
    assert any(edge["relation"] == "LOCATED_IN" for edge in graph.edges)
